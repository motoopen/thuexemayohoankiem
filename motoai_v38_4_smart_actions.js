/* motoai_v38_4_smart_actions.js
   ✅ UI UPGRADE: v38.4
      + [NEW] Smart Actions Bar: Thanh hành động thông minh dưới header (#mta-actions).
      + Logic: Thanh trên tự biến đổi theo ngữ cảnh (Context-aware), thanh dưới giữ nguyên gợi ý.
      + Data: Cập nhật Map/Address Nguyễn Tú Long Biên.

   ✅ SECURITY & COMPATIBILITY: v38.3.3 (Inherited)
      + Hardening XSS & Unicode Regex Fallback.
*/
...
(function(){
  ...
  const DEF = {
    brand: "Nguyễn Tú",
    phone: "0942467674", // Số điện thoại chính
    zalo:  "",           // Để trống sẽ tự tạo link từ phone
    // Link Map chính xác của anh
    map:   "https://maps.app.goo.gl/2w4qijAfZ6ieoAjs6?g_st=ipc", 
    address: "112 Nguyễn Văn Cừ, Bồ Đề, Long Biên, Hà Nội",
    avatar: "👩‍💼",
    themeColor: "#0084FF",
  ...
  function stripMarkdown(s){
    if(!s) return s;
    let o = String(s);
    o = o.replace(/\*\*(.*?)\*\*/g, '$1').replace(/__(.*?)__/g, '$1').replace(/`([^`]+)`/g, '$1');
    o = o.replace(/^\s{0,3}#{1,6}\s+/gm, '').replace(/^\s{0,3}[-*+]\s+/gm, '');
    o = o.replace(/!\[[^\]]*]\([^)]+\)/g, '').replace(/\[([^\]]+)]\(([^)]+)\)/g, '$1 - $2');
  ...
  :root {
    ...
    --m-bg: #ffffff; --m-bg-sec: #f4f6f8; --m-text: #0b1220; --m-text-sub: #64748b;
    --m-border: rgba(0,0,0,0.08);
    --m-bubble-bg: linear-gradient(135deg, var(--m-blue), #00B2FF);
    --m-user-bg: var(--m-blue); --m-bot-bg: #fff; --m-bot-bd: rgba(0,0,0,0.06);
    --m-in-h: 36px; --m-in-fs: 16px;
  }
  @media (prefers-color-scheme: dark) {
    :root {
      --m-bg: #1e1e1e; --m-bg-sec: #2c2c2e; --m-text: #e0e0e0; --m-text-sub: #a0a0a0;
      --m-border: rgba(255,255,255,0.12); --m-bot-bg: #2c2c2e; --m-bot-bd: rgba(255,255,255,0.08);
    }
  }
  #mta-root { position: fixed; right: 20px; bottom: 20px; z-index: var(--mta-z); font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Helvetica, Arial, sans-serif; }
  #mta-bubble { width: 58px; height: 58px; border: none; border-radius: 50%; background: var(--m-bubble-bg); box-shadow: 0 8px 24px rgba(0, 132, 255, 0.3); display: flex; align-items: center; justify-content: center; cursor: pointer; color: #fff; font-size: 26px; transition: transform 0.2s; }
  #mta-bubble:hover { transform: scale(1.05); }
  #mta-backdrop { position: fixed; inset: 0; background: rgba(0,0,0,0.3); opacity: 0; pointer-events: none; transition: opacity 0.2s ease; backdrop-filter: blur(2px); }
  #mta-backdrop.show { opacity: 1; pointer-events: auto; }
  #mta-card { position: fixed; right: 20px; bottom: 20px; width: 380px; height: 70vh; max-height: 720px; background: var(--m-bg); color: var(--m-text); border-radius: 20px; box-shadow: 0 12px 40px rgba(0,0,0,0.2); display: flex; flex-direction: column; overflow: hidden; transform: translateY(120%) scale(0.95); transition: transform 0.3s cubic-bezier(0.32, 0.72, 0, 1); opacity: 0; pointer-events: none; }
  #mta-card.open { transform: translateY(0) scale(1); opacity: 1; pointer-events: auto; }
  
  /* HEADER */
  #mta-header { background: var(--m-bubble-bg); color: #fff; padding: 12px 16px; display: flex; align-items: center; gap: 10px; flex-shrink: 0; }
  #mta-header .avatar { width: 32px; height: 32px; border-radius: 50%; background: rgba(255,255,255,0.2); display: flex; align-items: center; justify-content: center; font-size: 16px; }
  ...
  #mta-header .close-btn { background: none; border: none; color: #fff; font-size: 24px; cursor: pointer; padding: 0 4px; }

  /* SMART ACTIONS BAR (Top) */
  #mta-actions { background: var(--m-bg); border-bottom: 1px solid var(--m-border); padding: 8px 12px; display: flex; gap: 8px; overflow-x: auto; white-space: nowrap; -webkit-overflow-scrolling: touch; transition: all 0.2s; }
  #mta-actions.hidden { display: none; }
  #mta-actions::-webkit-scrollbar { display: none; }
  #mta-actions button { flex-shrink: 0; border-radius: 999px; border: 1px solid var(--m-border); padding: 5px 12px; font-size: 12px; font-weight: 500; background: var(--m-bg-sec); color: var(--m-text); cursor: pointer; display: flex; align-items: center; gap: 4px; transition: background 0.15s; }
  #mta-actions button:active { opacity: 0.7; }

  /* BODY */
  #mta-body { flex: 1; overflow-y: auto; overflow-x: hidden; padding: 16px; background: var(--m-bg-sec); scroll-behavior: smooth; -webkit-overflow-scrolling: touch; }
  .m-msg { max-width: 80%; margin-bottom: 10px; padding: 10px 14px; border-radius: 18px; font-size: 15px; line-height: 1.4; word-wrap: break-word; }
  ...
  /* TAGS BAR (Bottom) */
  #mta-tags { background: var(--m-bg); border-top: 1px solid var(--m-border); padding: 8px 0; overflow-x: auto; white-space: nowrap; display: flex; gap: 8px; padding-left: 12px; transition: max-height 0.2s, opacity 0.2s; }
  #mta-tags.hidden { max-height: 0; opacity: 0; padding: 0; overflow: hidden; border: none; }
  #mta-tags::-webkit-scrollbar { display: none; }
  #mta-tags button { background: var(--m-bg-sec); color: var(--m-text); border: 1px solid var(--m-border); border-radius: 16px; padding: 6px 12px; font-size: 13px; cursor: pointer; flex-shrink: 0; }
  
  /* FOOTER */
  #mta-footer { background: var(--m-bg); padding: 10px 12px; border-top: 1px solid var(--m-border); display: flex; gap: 8px; align-items: center; padding-bottom: calc(10px + env(safe-area-inset-bottom, 0)); }
  #mta-in { flex: 1; height: var(--m-in-h); border: 1px solid var(--m-border); border-radius: 20px; padding: 0 16px; font-size: var(--m-in-fs); background: var(--m-bg-sec); color: var(--m-text); outline: none; transition: border-color 0.2s; appearance: none; -webkit-appearance: none; }
  #mta-in:focus { border-color: var(--m-blue); }
  #mta-send { width: 36px; height: 36px; border: none; border-radius: 50%; background: var(--m-blue); color: #fff; display: flex; align-items: center; justify-content: center; cursor: pointer; font-size: 16px; }
  #mta-typing { font-size: 12px; color: var(--m-text-sub); margin-bottom: 8px; margin-left: 4px; font-style: italic; }

  @media (max-width: 480px) {
    #mta-card { right: 0; left: 0; bottom: 0; width: 100%; height: 85vh; max-height: none; border-radius: 20px 20px 0 0; transform: translateY(100%); }
    #mta-card.open { transform: translateY(0); }
    #mta-footer { padding-bottom: calc(10px + env(safe-area-inset-bottom, 0)); }
  }
  `;

  // ✅ UPDATE HTML: Thêm #mta-actions dưới header
  const HTML = `
  <div id="mta-root">
    ...
    <div id="mta-card">
      <header id="mta-header">
        ...
        <div class="info">
          <div class="name">${esc(CFG.brand)}</div>
          <div class="status">Sẵn sàng hỗ trợ</div>
        </div>
        <button class="close-btn" id="mta-close">×</button>
      </header>

      <!-- 🔹 NEW: Thanh Smart Actions -->
      <div id="mta-actions" class="hidden"></div>

      <main id="mta-body"></main>
      
      <!-- 🔹 OLD: Thanh Tags gợi ý -->
      <div id="mta-tags">
  ...
  function renderSess(){
    ...
    else addMsg("bot", naturalize(`Xin chào 👋, em là nhân viên hỗ trợ của ${CFG.brand}. Anh/chị cần thuê xe số, xe ga hay tìm địa chỉ cửa hàng ạ?`));
  }
  ...
  /* ====== LOGIC SMART ACTIONS (V38.4) ====== */
  
  // 1. Phân tích ngữ cảnh câu chat cuối để tạo danh sách nút
  function buildSmartActionsFromContext(lastUserText){
    ...
    // Nhóm 1: Khách đang quan tâm xe/giá -> Mời xem ảnh, giữ xe
    if (intents.needPrice || type || qty) {
      acts.push({id:"hold",  label:"⭐ Giữ xe này"});
      acts.push({id:"photos",label:"🖼 Xem ảnh thật"});
      acts.push({id:"policy",label:"📄 Xem thủ tục"});
    }

    // Nhóm 2: Khách nhắc đến địa điểm/giao xe -> Mời chỉ đường
    if (/\b(map|địa chỉ|ở đâu|đến đâu|location|đường|ship|qua lấy)\b/i.test(q) || intents.needDelivery) {
      acts.push({id:"map",   label:"📍 Chỉ đường"});
    }

    // Nhóm 3: Mặc định luôn có liên hệ
    acts.push({id:"call",  label:"📞 Gọi ngay"});
    acts.push({id:"zalo",  label:"💬 Chat Zalo"});
  ...
  // 2. Render nút lên giao diện
  function renderSmartActions(actions, lastUserText){
    const bar = $("#mta-actions");
    if (!bar) return;
    bar.innerHTML = "";
    ...
    actions.forEach(act=>{
      ...
      btn.addEventListener("click", ()=> handleSmartActionClick(act.id));
      bar.appendChild(btn);
    });
  }

  // 3. Xử lý click (Hành động thật hoặc gửi tin nhắn mẫu)
  function handleSmartActionClick(id){
    switch(id){
      case "call":
        if (CFG.phone) location.href = "tel:"+CFG.phone;
        break;
      case "zalo":
        if (CFG.zalo)  window.open(CFG.zalo, "_blank");
        else if (CFG.phone) window.open("https://zalo.me/"+CFG.phone.replace(/\s+/g,""), "_blank");
        break;
      case "map":
        if (CFG.map) window.open(CFG.map, "_blank");
        else sendUser("Cho tôi xin địa chỉ cửa hàng và bản đồ");
        break;
      case "photos":
        sendUser("Gửi cho tôi xem ảnh thực tế các mẫu xe này");
        break;
      case "policy":
        sendUser("Thủ tục thuê xe và tiền cọc cụ thể ra sao?");
        break;
      case "hold":
        sendUser("Tôi muốn đặt trước xe này, giữ xe giúp tôi");
        break;
    }
  }
  ...
  function tk(s){
    ...
    try {
      ...
      return raw.replace(/[^a-z0-9\u00C0-\u1EF9\s]+/gi,' ').split(/\s+/).filter(Boolean);
    }
  }
  ...
  async function deepAnswer(userText){
    ...
    // Ưu tiên trả lời địa chỉ nếu hỏi trực tiếp
    if((/\b(địa chỉ|ở đâu|đến đâu|location)\b/i.test(q)) && CFG.address) return polite(`cửa hàng bên em ở: ${CFG.address}. Anh/chị bấm nút "Chỉ đường" bên trên để mở Google Maps nhé.`);
  ...
  // ✅ MODIFIED: Cập nhật Smart Actions mỗi lần khách chat
  async function sendUser(text){
    ...
    // 🔥 Trigger update Smart Actions
    const acts = buildSmartActionsFromContext(v);
    renderSmartActions(acts, v);
  ...
  function autoAvoid(){
    ...
    if (window.visualViewport && window.innerWidth < 600) {
      ...
      if (vh < window.innerHeight - 100) {
        card.style.bottom = `${Math.max(0, bottom)}px`; card.style.height = `${vh}px`; card.style.borderRadius = "0";
      } else {
        card.style.bottom = "0"; card.style.height = "85vh"; card.style.borderRadius = "20px 20px 0 0";
      }
    }
  }
  ...
  const _listeners = [];
  function addL(el, evt, fn, opts){
    if(el){ el.addEventListener(evt, fn, opts); _listeners.push({el, evt, fn}); }
  }
  function bindEvents(){
    addL($("#mta-bubble"), "click", openChat);
    addL($("#mta-backdrop"), "click", closeChat);
    addL($("#mta-close"), "click", closeChat);
    addL($("#mta-send"), "click", ()=>{
      const inp=$("#mta-in"); const v=inp.value.trim(); if(!v) return; inp.value=""; sendUser(v);
    });
    addL($("#mta-in"), "keydown", e=>{
      if(e.key==="Enter" && !e.shiftKey){ e.preventDefault(); const v=e.target.value.trim(); if(!v) return; e.target.value=""; sendUser(v); }
      // Khi gõ phím thì ẩn Tag Bar dưới để đỡ rối, nhưng Smart Actions trên thì giữ nguyên
      const tags=$("#mta-tags"); 
      if(tags){ if(e.target.value.trim().length>0) tags.classList.add('hidden'); else tags.classList.remove('hidden'); }
    });
    const track=$("#mta-tags");
    if(track){ track.querySelectorAll("button").forEach(btn=> addL(btn, "click", ()=> sendUser(btn.dataset.q||btn.textContent))); }

    if(window.visualViewport){
      addL(window.visualViewport, "resize", autoAvoid, {passive:true});
      addL(window.visualViewport, "scroll", autoAvoid, {passive:true});
    }
  }
  ...
  window.MotoAI_v38 = {
    ...
    destroy: ()=> {
      _listeners.forEach(l => l.el.removeEventListener(l.evt, l.fn));
      $("#mta-root")?.remove();
      window.MotoAI_v38_LOADED = false;
    }
  };
})();

/* motoai_v38_3_3_autodock.js
   ✅ UI UPGRADE: v38.3 + Auto Dock
      ...
      + [NEW] Auto Dock: Bubble tự động nhảy lên trên Quick Call / Fixed Footer
...
(function(){
  ...
  // ====== AUTO DOCK ABOVE QUICK CALL / FOOTER NAV ======
  // Các selector có thể gặp cho nút gọi nhanh / footer nav
    ...
    try{
      ...
      DOCK_SELECTORS.forEach(sel=>{
        document.querySelectorAll(sel).forEach(el=>{
          ...
          // Bỏ qua nếu không nằm trong viewport
          if(r.bottom <= 0 || r.top >= window.innerHeight) return;

          // Chỉ quan tâm các phần tử đang "dính" đáy màn hình (<= 80px từ mép dưới)
          const distFromBottom = window.innerHeight - r.bottom;
          if(distFromBottom < 80){
            // Đẩy chatbot cao hơn phần tử này một đoạn 16px
            extra = Math.max(extra, r.height + 16);
          }
        });
      });

      const base = 20; // khoảng cách mặc định
      const bottom = base + extra;
      document.documentElement.style.setProperty("--mta-bottom", bottom + "px");
    }catch(e){
      // tránh crash toàn bộ nếu lỗi
    }
  }
  ...
  #mta-root {
    position: fixed;
    right: 20px;
    bottom: var(--mta-bottom, 20px); /* ✔ dùng biến, mặc định 20px */
    z-index: var(--mta-z);
  ...
  #mta-card {
    position: fixed;
    right: 20px;
    bottom: var(--mta-bottom, 20px); /* ✔ đồng bộ với bubble khi đóng */
    ...
    transition: transform 0.3s cubic-bezier(0.32, 0.72, 0, 1), bottom 0.3s ease;
  ...
  // 🔹 FIXED: Sửa lỗi cú pháp sort((a,b)=>b.score - a.score)
  ...
  ready(async ()=>{
    ...
    // 🔹 Căn lại vị trí chatbot cho phù hợp quick call + footer nav
    updateDockOffset();
    window.addEventListener("resize", updateDockOffset);
    window.addEventListener("scroll", updateDockOffset);

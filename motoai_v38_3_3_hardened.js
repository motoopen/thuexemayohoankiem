/* motoai_v38_3_3_hardened.js
   ✅ UI UPGRADE: v38.3
      + Fix Input Zoom iOS
      + VisualViewport Logic
      + Auto Dark/Light Mode

   ✅ SECURITY & COMPATIBILITY PATCH: v38.3.3
      + [2.1] Hardening XSS: Escaping HTML Config Injection
      + [2.2] Cross-Browser: Regex Unicode Fallback (Safe for old Chrome/Safari)
      + [2.3] Performance: Passive Event Listeners & Storage Safety Wrappers
*/
...
(function(){
  ...
  // ✅ PATCH [2.1]: Helper Escape HTML (Prevent XSS via Config)
  ...
  // ✅ PATCH [2.1]: Applied esc() to brand & avatar
  const HTML = `
  <div id="mta-root">
  ...
  /* ====== INDEX / SEARCH ====== */
  // ✅ PATCH [2.2]: Tokenizer an toàn (Safe for browsers that don't support \p{L})
  function tk(s){
    const raw = (s||"").toLowerCase();
    try {
      // Use new RegExp to avoid parse-time SyntaxError on old engines
      const re = new RegExp('[^\\p{L}\\p{N}\\s]+', 'gu');
      return raw.normalize('NFC').replace(re,' ').split(/\s+/).filter(Boolean);
    } catch(e) {
      // Fallback: ASCII + Basic Vietnamese Range
          ...
          if(r && r.ok){
            ...
            for(const u of ds){
              ...
              if(/\.txt($|\?)/i.test(u)){
                ...
                if(CFG.smart.autoPriceLearn){
                  // ✅ PATCH [2.3]: Try-Catch wrapper for autoPriceLearn
                  try {
                    const autos = extractPricesFromText(txt);
                    if(autos && autos.length){
                      stats.autoPriceHits += autos.length;
  ...
  // ✅ PATCH [2.3]: Cleanup listeners & Passive flag
  const _listeners = [];

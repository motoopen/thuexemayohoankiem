/* motoai_v38_3_complete_release.js
   ✅ UI UPGRADE: v38.3 (Input Zoom iOS, VisualViewport, Dark Mode, Bottom Sheet)
   ✅ LOGIC UPGRADE: Semantic Re-ranking + Advanced Multi-Order Reasoning
   
   - Semantic Rerank: BM25 + Char n-gram Jaccard score.
   - Reasoning: Parse "2 xe số + 1 vision 3 ngày" -> Tính tổng giá.
   - AutoLearn: ưu tiên moto_sitemap.json; fallback sitemap.xml
*/
...
(function(){
  ...
  const DEF = {
    ...
    // Smart flags
    smart: {
      semanticSearch:    true,   // giữ nguyên
      semanticRerank:    true,   // 🔹 MỚI: bật semantic re-ranking
      extractiveQA:      true,
      autoPriceLearn:    true,
      advancedReasoning: true    // 🔹 MỚI: bật reasoning nâng cao
    },
    debug: true
  };
  ...
  /* ====== LOGIC CHATBOT (Giữ nguyên v38.1) ====== */
  const TYPE_MAP = [
  ...
  /* ====== SIMPLE INDEX + BM25 mini + SEMANTIC RE-RANK ====== */
  ...
  /* ====== SEMANTIC RE-RANK (char n-gram) ====== */
  function ngrams(str, n){
    const s = (str||"").toLowerCase().normalize("NFC").replace(/\s+/g," ");
    const out = new Set();
    if(!s) return out;
    const len = s.length;
    const N = n||3;
  ...
  // ⬇️ Chỉ dùng title + phần đầu mô tả để tránh nặng
  function semanticScore(query, meta, text){
    if(!query) return 0;
    const qSet = ngrams(query, 3);
    const target = ((meta && meta.title) ? meta.title + " " : "") + (text||"");
    const tSet = ngrams(target.slice(0, 260), 3);
    return jaccard(qSet, tSet); // 0 → 1
  }

  /* Nâng cấp searchIndex để dùng semantic re-ranking */
  function searchIndex(query, k=3){
    ...
    // B1: tính điểm BM25 hoặc keyword cơ bản
    let scored = bm
      ? docs.map(d=>{
          const len  = tk(d.text).length || 1;
          const base = bm.score(query, d.id, len);
          return { bmScore: base, meta: d.meta, text: d.text };
        }).filter(x=> x.bmScore > 0)
      : idx.map(it=>{
          const allToks = tk((it.title||"")+" "+(it.text||""));
          const hit     = tk(query);
          const base    = allToks.filter(t=> hit.includes(t)).length;
          return { bmScore: base, meta: it, text: (it.title||"")+" "+(it.text||"") };
        }).filter(x=> x.bmScore > 0);

    if(!scored.length) return [];

    // B2: SEMANTIC RE-RANK (không cắt bỏ BM25, chỉ cộng thêm điểm)
    if(CFG.smart.semanticRerank){
      scored.forEach(item=>{
        const sem = semanticScore(query, item.meta, item.text);
        // trọng số: BM25 + 3 * semantic
        item.totalScore = (item.bmScore || 0) + sem * 3;
      });
      scored.sort((a,b)=> (b.totalScore||0) - (a.totalScore||0));
    } else {
      scored.sort((a,b)=> b.bmScore - a.bmScore);
    }

    return scored.slice(0, k).map(x=> x.meta);
  }
  ...
  async function learnSites(origins, force){
    ...
    if(CFG.debug){
      try{
        ...
        if(rows.length){
          console.groupCollapsed("%cMotoAI v38.3 — Learn Summary","color:"+CFG.themeColor+";font-weight:bold");
  ...
  /* ====== ADVANCED ORDER PARSER (multi-bike) ====== */
  function parseMultiOrder(text){
    const raw = String(text||"");
    const lower = raw.toLowerCase();
    const items = [];

    const patterns = [
      { re:/(\d+)\s*(?:xe\s*)?(số|wave|blade|sirius|jupiter|future|dream)/gi, type:'xe số' },
      { re:/(\d+)\s*(?:xe\s*)?(ga|vision|air\s*blade|airblade|lead|liberty|vespa|grande|janus|sh)/gi, type:'xe ga' },
      { re:/(\d+)\s*(?:xe\s*)?(50\s*cc|50cc)/gi, type:'50cc' },
      { re:/(\d+)\s*(?:xe\s*)?(điện|vinfast|yadea|dibao|gogo|klara|evo)/gi, type:'xe điện' },
      { re:/(\d+)\s*(?:xe\s*)?(côn|côn tay|tay côn|exciter|winner|raider|cb150|cbf190|w175|msx)/gi, type:'xe côn tay' }
    ];

    patterns.forEach(p=>{
      let m;
      while((m = p.re.exec(lower))){
        const cnt = parseInt(m[1], 10) || 0;
        if(!cnt) continue;
        const exist = items.find(it=> it.type === p.type);
        if(exist) exist.count += cnt;
        else items.push({ type: p.type, count: cnt });
      }
    });

    // Nếu không bắt được pattern "n xe ...", fallback về detectType cũ
    if(!items.length){
      const t = detectType(raw);
      if(t) items.push({ type: t, count: 1 });
    }

    const qty = detectQty(raw);
    return { items, qty };
  }

  function computeQuote(order){
    if(!order || !order.qty || !order.items || !order.items.length) return null;
    const qty = order.qty;
    ...
    order.items.forEach(it=>{
      const base = baseFor(it.type, qty.unit);
      if(!base) return;
      const count = it.count || 1;
      const sub = base * qty.n * count;
      total += sub;
      lines.push(`${count} ${it.type} x ${qty.n} ${unitLabel} ~ ${nfVND(sub)}đ`);
    });

    if(!lines.length) return null;

    let text = `Báo giá nhanh cho anh/chị:\n- ${lines.join('\n- ')}`;
    text += `\nTổng khoảng: ${nfVND(total)}đ.`;
    if(qty.unit==="ngày" && qty.n>=3){
      text += ` Nếu thuê theo tuần có thể tối ưu chi phí hơn.`;
    }
    text += ` Anh/chị cần em giữ xe và gửi ảnh xe qua Zalo ${CFG.phone} không?`;

    return naturalize(text);
  }
  ...
  async function deepAnswer(userText){
    ...
    // 🔹 Reasoning nâng cao cho báo giá nhiều xe
    if(intents.needPrice){
      if(CFG.smart.advancedReasoning){
        const order = parseMultiOrder(q);
        // Thử báo giá nâng cao trước (nhiều xe, nhiều loại)
        const adv = computeQuote(order);
        if(adv) return polite(adv);
      }
      // Fallback về logic cũ nếu không bắt được multi-order
  ...
  /* ✅ V38.3 LOGIC: Visual Viewport Auto Avoid */
  function autoAvoid(){
    if (!isOpen) return;
    const card = $("#mta-card");

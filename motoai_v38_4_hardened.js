/* motoai_v38_4_hardened.js
   ✅ SECURITY & COMPATIBILITY UPGRADE: v38.4
      1. Hardening XSS: Escaping HTML configs (brand, avatar).
      2. Cross-Browser Regex: Fallback cho trình duyệt cũ (Chrome <64, Safari cũ).
      3. Storage Safety: Bọc try/catch cho mọi tác vụ localStorage (Private Mode support).
      4. Event Cleanup: Thêm phương thức destroy() và passive listeners (SPA Friendly).

   ✅ LOGIC KEEP: v38.3 (VisualViewport + DarkMode + Markdown Strip)
*/
...
(function(){
  ...
  // ✅ PATCH 2.1: Helper Escape HTML (Chống XSS tại Config)
  ...
  // ✅ PATCH 2.1: Áp dụng esc() vào Template String
  const HTML = `
  <div id="mta-root">
  ...
  // ✅ PATCH 2.2: Cross-Browser Regex Fallback
  function tk(s){
    const raw = (s||"").toLowerCase();
    try {
      return raw.normalize('NFC').replace(/[^\p{L}\p{N}\s]+/gu,' ').split(/\s+/).filter(Boolean);
    } catch(e) {
      // Fallback for older browsers (no unicode property escape support)
      ...
      if(CFG.smart.autoPriceLearn){
        try{
          ...
          if(autos && autos.length){
            ...
            // ✅ PATCH 2.3: Storage Safety Check
            try{ localStorage.setItem(K.autoprices, JSON.stringify(stash.slice(-500))); }catch{}
          }
        }catch{}
      }
  ...
  // ✅ PATCH 2.3: Event Listener Cleanup & Passive
  function bindEvents(){
    ...
    // VisualViewport Events with Passive
  ...
  // Helper cleanup cho SPA
  function cleanup(){
    try {
      if(window.visualViewport) {
        window.visualViewport.removeEventListener("resize", autoAvoid);
        window.visualViewport.removeEventListener("scroll", autoAvoid);
      }
      const root = $("#mta-root"); if(root) root.remove();
  ...
  window.MotoAI_v38 = {
    ...
    destroy: cleanup // Expose destroy for SPA cleanup
  };
})();

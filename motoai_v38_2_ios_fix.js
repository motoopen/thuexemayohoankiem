/* motoai_v38_2_ios_fix.js
  ✅ GỘP: v27 (BM25 + Extractive QA + Auto-Price Learn + Multi-site)
       + v37.6 (UI nhỏ như Messenger + DeepContext + ưu tiên moto_sitemap.json)
       + v38.1 (Debug counters + console.table, fix UI input)
       + v38.2 (IOS SAFE FIX: Uses CSS Variables for bottom offset, respects safe-area-inset-bottom, handles visualViewport)
  ...
  - UI: ô nhập cao ~32px, tương phản tốt cả light/dark
  - iOS Fix: Không ghi đè style.bottom, dùng CSS Var --m-kb-offset để đẩy UI lên khi có phím ảo.
...
(function(){
 ...
 :root{
   ...
   /* iOS Safe Area & Keyboard Variables */
   --m-kb-offset: 0px; /* JS sẽ cập nhật biến này khi bàn phím bật */
 }
 #mta-root{
   position:fixed; right:16px;
   /* FIX: Không dùng JS ghi đè bottom. Dùng calc để cộng dồn safe-area và keyboard offset */
   bottom: calc(16px + env(safe-area-inset-bottom, 0px) + var(--m-kb-offset));
   z-index:var(--mta-z);
   font-family:-apple-system,system-ui,Segoe UI,Roboto,"Helvetica Neue",Arial,sans-serif;
   transition: bottom 0.2s cubic-bezier(0.1, 0.7, 1.0, 0.1); /* Smooth nhảy nhẹ */
 }
 #mta-bubble{
 ...
 #mta-card{
   position:fixed; right:16px;
   /* Card cũng phải tôn trọng safe area và keyboard offset */
   bottom: calc(16px + env(safe-area-inset-bottom, 0px) + var(--m-kb-offset));
 ...
 #mta-input{
   ...
   /* Sticky bottom để không bị trôi khi body scroll, nhưng vị trí tổng thể do #mta-card quyết định */
 ...
 @media(max-width:520px){
   #mta-card{
       ...
       /* Trên mobile, giảm chiều cao nếu bàn phím mở để tránh che khuất */
       height:70vh;
   }
   /* Nếu có offset bàn phím lớn, giảm chiều cao card để vừa với phần màn hình còn lại */
   body.mta-kb-open #mta-card {
       height: calc(100vh - 120px - var(--m-kb-offset));
   }
 ...
 function openChat(){
   ...
   // iOS fix: Check viewport ngay khi mở
   autoAvoid();
 }
 function closeChat(){
   if(!isOpen) return;
   ...
   // iOS fix: Reset offset khi đóng
   const root=$("#mta-root");
   if(root) root.style.setProperty('--m-kb-offset', '0px');
 }
 ...
 function autoAvoid(){
   ...
   // 1. Tính toán Blocker (Nút gọi nhanh, v.v...)
   let blockerOffset = 0;
   ...
   if(blockers){
     ...
     if(gap < 140) blockerOffset = gap + 16; // Chừa khoảng 16px trên nút
   }

   // 2. Tính toán Keyboard (Visual Viewport trên iOS)
   let kbOffset = 0;
   if(window.visualViewport){
     const vv = window.visualViewport;
     const hDiff = window.innerHeight - vv.height;
     // Chỉ khi bàn phím mở, hDiff sẽ lớn đáng kể (>150px)
     // Nếu chỉ có thanh địa chỉ co giãn (~50-80px), ta bỏ qua
     if(isOpen && hDiff > 140) {
        kbOffset = hDiff;
   ...
   // 3. Cập nhật CSS Variable (quan trọng: không ghi đè bottom trực tiếp)
   // Lấy max giữa blocker và keyboard để không bị cộng dồn vô lý
   const totalOffset = Math.max(0, blockerOffset, kbOffset);
   root.style.setProperty('--m-kb-offset', totalOffset + "px");

   // 4. Scroll body chat xuống cuối nếu đang mở
   if(isOpen && totalOffset > 0){
      ensureInputVisible();
   }
 }
 ...
 function bindEvents(){
   ...
   // iOS Focus Fix: Force check visual viewport khi focus
   inp.addEventListener("focus", () => { setTimeout(autoAvoid, 100); setTimeout(autoAvoid, 500); });
   inp.addEventListener("blur", () => { setTimeout(autoAvoid, 100); });
   ...
   if(CFG.autolearn){
     try{
       ...
       if(!last || (Date.now()-last) >= CFG.refreshHours*3600*1000){
         if(CFG.debug) console.groupCollapsed("%cMotoAI v38.2 — AutoLearn start","color:"+CFG.themeColor+";font-weight:bold");
